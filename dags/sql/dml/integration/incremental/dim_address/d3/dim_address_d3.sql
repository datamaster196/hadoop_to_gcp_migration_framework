create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_address_d3_work_source` as
select
distinct
coalesce(trim(calr_bsc_ad),'') as raw_address_1_nm,
coalesce(trim(calr_supl_ad),'') as raw_address_2_nm,
cast('' as string) as raw_care_of_nm,
coalesce(trim(calr_cty_nm),'') as raw_city_nm,
coalesce(trim(calr_st_cd),'') as raw_state_cd,
coalesce(trim(calr_zip_cd),'') as raw_postal_cd,
...
'd3' address_valid_typ,
current_datetime,
{{ dag_run.id }} as  batch,
current_datetime,
{{ dag_run.id }} as  batch
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_address_d3_work_source`
