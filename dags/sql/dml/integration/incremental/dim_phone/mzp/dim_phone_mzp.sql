create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_phone_mzp_work_source` as
select
distinct
coalesce(TRIM(phone),'')  as raw_phone_nbr,
`{{ var.value.INTEGRATION_PROJECT }}.udfs.hash_phone`(coalesce(TRIM(phone),'')) as phone_adw_key
...
current_datetime,
{{ dag_run.id }} as  batch
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_phone_mzp_work_source`
