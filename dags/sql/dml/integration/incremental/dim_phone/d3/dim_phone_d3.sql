create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_phone_d3_work_source` as
select
distinct
coalesce(trim(tel_nr_ext),'') as raw_phone_nbr,
...
current_datetime,
{{ dag_run.id }} as  batch
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_phone_d3_work_source`
