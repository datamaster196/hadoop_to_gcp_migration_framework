create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_email_epic_work_source` as
select
distinct
    coalesce(TRIM(emailweb),'') AS raw_email_nm,
`{{ var.value.INTEGRATION_PROJECT }}.udfs.hash_email`(coalesce(TRIM(emailweb),'')) as email_adw_key
...
'epic' email_valid_typ,
current_datetime,
{{ dag_run.id }} as  batch,
current_datetime,
{{ dag_run.id }} as  batch
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_email_epic_work_source`
