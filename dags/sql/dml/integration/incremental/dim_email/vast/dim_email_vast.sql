create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_email_vast_work_source` as
select
distinct
    coalesce(TRIM(EMAILADDRESS),'') AS raw_email_nm,
...
'vast' email_valid_typ,
current_datetime,
{{ dag_run.id }} as  batch,
current_datetime,
{{ dag_run.id }} as  batch
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.dim_email_vast_work_source`
