...
SELECT
  ...
  LAST_UPD_DT,
  DATA_SOURCE,
  ROW_NUMBER() OVER(PARTITION BY sales_agent_source.SALES_AGENT_ACTIVITY_KY,  sales_agent_source.data_source ORDER BY sales_agent_source.last_upd_dt desc) AS dupe_check
FROM (
  SELECT
    MEMBER_KY,
    PAYMENT_DETAIL_KY,
...
WHERE
  CAST(sales_agent_source.LAST_UPD_DT AS datetime) > (
  SELECT
