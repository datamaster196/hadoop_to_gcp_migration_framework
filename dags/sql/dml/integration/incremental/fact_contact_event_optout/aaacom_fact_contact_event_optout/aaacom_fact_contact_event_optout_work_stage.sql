CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.aaacom_fact_contact_event_optout_work_stage` AS
SELECT
  ...
  source.event_dtm
FROM
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.aaacom_fact_contact_event_optout_work_transform` source
LEFT JOIN
...
ON
  ...
  AND target.system_of_record='AAACOM'
WHERE
  source.dupe_check=1
  AND
