CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_contact_source_key_stage` AS
WITH
  normalized_source_key AS (
  SELECT
    contact_adw_key,
    'membership_key' AS key_typ_nm,
  ...
  SELECT
    contact_adw_key,
    'derived_member_key' AS key_typ_nm,
    CAST(last_upd_dt AS datetime) last_upd_dt,
    derived_member_ky AS source_1_key,
    CAST(NULL AS string) AS source_2_key,
    CAST(NULL AS string) AS source_3_key,
    CAST(NULL AS string) AS source_4_key,
    CAST(NULL AS string) AS source_5_key,
    derived_member_ky AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_contact_stage`
  UNION DISTINCT
  SELECT
  ...
  SELECT
    ...
    case
        ...
        when key_typ_nm like 'membership_key%' then 'membership_key'
        when key_typ_nm like 'member_key%' then 'member_key'
		when key_typ_nm like 'derived_member_key%' then 'derived_member_key'
    else 'customer_id'
  END
    AS key_typ_nm,
  ...
  SELECT
    contact_adw_key,
    'mzp' AS contact_source_system_nm,
    key_typ_nm,
    effective_start_datetime,
    CASE
...
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_contact_source_key_stage` source
ON
