CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_contact_name_stage` AS
WITH
...
MERGE INTO
  `{{ var.value.INTEGRATION_PROJECT }}.adw.xref_contact_name` target
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_contact_name_stage` source
ON
  ...
  WHEN NOT MATCHED BY SOURCE AND target.contact_source_system_nm='mzp' THEN DELETE
;
