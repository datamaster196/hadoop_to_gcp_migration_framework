create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_phone_stage` as
...
where phone_typ_cd is not null
union distinct
select
target.contact_adw_key,
substr(target.phone_typ_cd,1,case when strpos(target.phone_typ_cd,'-')>0 then strpos(target.phone_typ_cd,'-')-1 else length(target.phone_typ_cd) end) as phone_typ_cd,
...
row_number() over(partition by contact_adw_key, phone_typ_cd,  effective_start_datetime order by effective_end_datetime desc,phone_adw_key ) as rn
from deduped
)
select
contact_adw_key,
...
using `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_phone_stage` source
