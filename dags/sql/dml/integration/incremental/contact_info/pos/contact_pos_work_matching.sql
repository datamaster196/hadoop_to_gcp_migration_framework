...
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_matched` as
select
coalesce(member_id_match.contact_adw_key,
        address_match.contact_adw_key,
        zip5_match.contact_adw_key,
        pos_cust_id_match.contact_adw_key) as contact_adw_key,
    ...
    (cast(last_upd_dt as datetime) between effective_start_datetime and effective_end_datetime) or (mbr_id_count = 1)
     ...
     LEFT OUTER JOIN (
       SELECT
         ...
         , cleansed_state_cd
       FROM
         `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_target_work`
     ...
     LEFT OUTER JOIN (
      ...
      -- Matching Criteria 4: POS Customer ID--       
left outer join (select max(contact_adw_key) as contact_adw_key
                      , pos_cust_id
                      from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_target_work`
                      where coalesce(pos_cust_id,'') != ''
                      group by pos_cust_id
                      ) pos_cust_id_match 
                      on source.pos_cust_id=pos_cust_id_match.pos_cust_id
;
--self matching on source key
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_1` as
with member_guid as
(select
...
FROM `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_matched` original
left outer join member_guid b on 
original.pos_cust_id=b.pos_cust_id
where original.contact_adw_key is null;
--self matching on membership_id
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_2` as
...
from (select distinct 
            ...
            membership_id,
            associate_id
            from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_1`
...
(
    select
        ...
        from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_1` a
...
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_1` original
...
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_3` as
...
(
         SELECT
           ...
           cleansed_state_cd
         FROM
           `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_2`
         WHERE
               cleansed_first_nm      != ''
       ...
       SELECT
         ...
         MIN(b.contact_adw_key) AS new_contact_adw_key
       FROM
         `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_2` a
       JOIN
         address_guid b
       ON
...
original.cleansed_state_cd,
original.cleansed_postal_cd   
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_2` original
...
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_4` as
with address_guid as
(
         SELECT
           DISTINCT 
           ...
           cleansed_postal_cd
         FROM
           `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_3`
         WHERE
               cleansed_first_nm      != ''
           and cleansed_last_nm       != ''
           and cleansed_address_1_nm   != ''
       ...
       SELECT
         ...
         MIN(b.contact_adw_key) AS new_contact_adw_key
       FROM
         `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_3` a
       JOIN
         address_guid b
       ON
           a.cleansed_first_nm       = b.cleansed_first_nm
...
original.cleansed_state_cd,
original.cleansed_postal_cd  
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_3` original
left outer join matched_guid matched on original.contact_adw_key=matched.orig_contact_adw_key
;
--final staging
create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_stage` as 
select 
contact_adw_key ,
pos_cust_id,
iso_cd,
...
cleansed_state_cd,
cleansed_postal_cd 
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_unmatched_work_4`