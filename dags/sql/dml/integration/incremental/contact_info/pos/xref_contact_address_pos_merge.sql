create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_address_stage` as
...
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_stage`
where address_typ_cd is not null
union distinct
select
target.contact_adw_key,
substr(target.address_typ_cd,1,case when strpos(target.address_typ_cd,'-')>0 then strpos(target.address_typ_cd,'-')-1 else length(target.address_typ_cd) end) as address_typ_cd,
...
where contact_source_system_nm='pos'
),
cust_address as
(select
contact_adw_key,
...
lag(address_adw_key) over (partition by contact_adw_key, address_typ_cd order by cast(last_upd_dt as datetime), address_adw_key) as prev_address,
coalesce(lead(cast(last_upd_dt as datetime)) over (partition by contact_adw_key, address_typ_cd order by cast(last_upd_dt as datetime), address_adw_key), datetime('9999-12-31'
) )as next_record_date
...
address_typ_cd,
address_adw_key,
sum(new_address_tag) over (partition by contact_adw_key, address_typ_cd order by effective_start_datetime, address_adw_key) as grouping_column,
effective_start_datetime,
next_record_date
...
row_number() over(partition by contact_adw_key, address_typ_cd,  effective_start_datetime order by effective_end_datetime desc,address_adw_key ) as rn
from deduped
)
select
contact_adw_key,
...
concat(address_typ_cd, case when rn=1 then '' else concat('-',cast(rn as string)) end ) as address_type,
effective_start_datetime,
...
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.pos_contact_address_stage` source
ON
...
END
  ...
  AND target.contact_source_system_nm='pos' THEN
DELETE;
