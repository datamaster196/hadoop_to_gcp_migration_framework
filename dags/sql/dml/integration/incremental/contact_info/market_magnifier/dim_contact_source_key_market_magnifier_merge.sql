CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_source_key_stage` AS
WITH
  ...
  SELECT
    ...
    lexid AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_stage`
  UNION ALL
  SELECT
    contact_adw_key,
    'lexhhid' AS key_typ_nm,
    CAST(last_upd_dt AS datetime) last_upd_dt,
    lexhhid AS source_1_key,
    CAST(NULL AS string) AS source_2_key,
    CAST(NULL AS string) AS source_3_key,
    CAST(NULL AS string) AS source_4_key,
    CAST(NULL AS string) AS source_5_key,
    lexhhid AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_stage`
  UNION ALL
  SELECT
    ...
    cast(check_digit_nr as string) AS source_5_key,
    CONCAT(ifnull(CAST(iso_cd AS string),
        ''),ifnull(club_cd,
        ''),ifnull(membership_id,
        ''),ifnull(associate_id,
        ''),ifnull(cast(check_digit_nr as string),
        '')) AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_stage`
 ),
  ...
  SELECT
    contact_adw_key,
    case 
        when key_typ_nm like 'lexid%' then 'lexid'
        when key_typ_nm like 'lexhhid%' then 'lexhhid'
  ...
  SELECT
    contact_adw_key,
    'market_magnifier' AS contact_source_system_nm,
    key_typ_nm,
    effective_start_datetime,
...
using `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_source_key_stage` source
