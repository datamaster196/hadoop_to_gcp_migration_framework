...
from
(select
'market_magnifier' as source_system_nm,
'target' as match_system,
'source_key_match' as match_type,
source.lexid as source_system_key,
client_key_match.contact_adw_key as contact_adw_key
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_source_work` source      
      -- Matching Criteria 1: Member Key --
join (select max(contact_adw_key) as contact_adw_key
      , lexid
      from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_target_work`
      group by lexid
      ) client_key_match
      on source.lexid = client_key_match.lexid
union all
select
'market_magnifier' as source_system_nm,
'target' as match_system,
...
source.lexid as source_system_key,
member_id_match.contact_adw_key as contact_adw_key
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_unmatched_work_1` source    
      -- Matching Criteria 2: First Name, Last Name, Address Line 1, City, State, Name Suffix 
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      , effective_start_datetime
      , effective_end_datetime
      from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_unmatched_work_1` mk1
      join `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_contact_source_key` cs1 
      on mk1.lexid=cs1.source_1_key and 
      cs1.contact_source_system_nm='market_magnifier' and
      cs1.key_typ_nm='uniqentity_key'
      group by
      club_cd
      , membership_id
      , associate_id
...
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      and source.cleansed_city_nm       = address_match.cleansed_city_nm         
      and source.cleansed_state_cd      = address_match.cleansed_state_cd
	  and source.lexid     != address_match.lexid	  
      and source.cleansed_first_nm      != ''                          
      and source.cleansed_last_nm       != ''                                    
      and source.cleansed_address_1_nm  != ''                           
      and source.cleansed_city_nm       != ''                          
...
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      and source.cleansed_address_1_nm   = zip5_match.cleansed_address_1_nm
      and source.cleansed_postal_cd      = zip5_match.cleansed_postal_cd
	  and source.lexid     != zip5_match.lexid
      and source.cleansed_first_nm      != ''
      and source.cleansed_last_nm       != ''                 
      and source.cleansed_address_1_nm  != ''
      and source.cleansed_postal_cd     != ''
