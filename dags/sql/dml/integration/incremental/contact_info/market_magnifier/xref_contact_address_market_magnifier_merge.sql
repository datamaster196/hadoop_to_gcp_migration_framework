create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_address_stage` as
...
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.market_magnifier_contact_address_stage` source
ON
...
END
  ...
  AND target.contact_source_system_nm='market_magnifier' THEN
DELETE;
