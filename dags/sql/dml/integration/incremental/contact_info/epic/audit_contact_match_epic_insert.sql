...
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_source_work` source      
      -- Matching Criteria 1: Member Key --
join (select max(contact_adw_key) as contact_adw_key
      , client_unique_entity
      from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_target_work`
...
source.client_unique_entity as source_system_key,
member_id_match.contact_adw_key as contact_adw_key
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_unmatched_work_1` source    
      -- Matching Criteria 2: First Name, Last Name, Address Line 1, City, State, Name Suffix 
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      , effective_start_datetime
      , effective_end_datetime
      from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_unmatched_work_1` mk1
      join `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_contact_source_key` cs1 
      on mk1.client_unique_entity=cs1.source_1_key and 
      cs1.contact_source_system_nm='epic' and
