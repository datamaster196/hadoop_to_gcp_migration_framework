CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_name_stage` AS
WITH
  combined_cust_name AS (
  SELECT
    contact_adw_key,
    nm_adw_key,
  ...
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw.xref_contact_name` target
  WHERE
  ...
  SELECT
    ...
    coalesce(LEAD(CAST(last_upd_dt AS datetime)) OVER (PARTITION BY contact_adw_key ORDER BY CAST(last_upd_dt AS datetime), nm_adw_key),
  ...
  SELECT
    ...
    nm_adw_key,
    CASE
      WHEN prev_name IS NULL OR prev_name<>nm_adw_key THEN 1
    ELSE
    0
  END
    AS new_name_tag,
...
MERGE INTO
  `{{ var.value.INTEGRATION_PROJECT }}.adw.xref_contact_name` target
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.epic_contact_name_stage` source
ON
  target.contact_adw_key=source.contact_adw_key
  AND target.contact_source_system_nm=source.source_system
  AND target.nm_typ_cd=source.name_type
  ...
  SET target.nm_adw_key=source.nm_adw_key, 
  ...
  WHEN NOT MATCHED BY SOURCE AND target.contact_source_system_nm='epic' THEN DELETE
;
