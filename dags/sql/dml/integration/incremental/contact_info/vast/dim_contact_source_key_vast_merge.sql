CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_source_key_stage` AS
WITH
  normalized_source_key AS (
  SELECT
    contact_adw_key,
    'vast_cust_nbr' AS key_typ_nm,
    CAST(last_upd_dt AS datetime) last_upd_dt,
    vast_cust_nbr AS source_1_key,
    company AS source_2_key,
    CAST(NULL AS string) AS source_3_key,
    CAST(NULL AS string) AS source_4_key,
    CAST(NULL AS string) AS source_5_key,
    CONCAT(ifnull(CAST(vast_cust_nbr AS string),''),'|',ifnull(company,'')) AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
  UNION ALL
  SELECT
    contact_adw_key,
  ...
  SELECT
    contact_adw_key,
    case 
        when key_typ_nm like 'vast_cust_nbr%' then 'vast_cust_nbr'
  ...
  END
    ...
    ELSE
    CONCAT(ifnull(CAST(source_1_key AS string), ''),'|',ifnull(CAST(source_2_key AS string), ''))
  END
    AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_contact_source_key`
  WHERE
    contact_source_system_nm = 'vast'
  UNION DISTINCT
  SELECT
    *
  FROM
  ...
  SELECT
    contact_adw_key,
    'vast' AS contact_source_system_nm,
    key_typ_nm,
    effective_start_datetime,
...
using `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_source_key_stage` source
