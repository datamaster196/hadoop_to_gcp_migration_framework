CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_email_stage` AS
WITH
...
MERGE INTO
  `{{ var.value.INTEGRATION_PROJECT }}.adw.xref_contact_email` target
USING
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_email_stage` source
ON
  ...
  ( source.contact_adw_key, 
	...
	source.batch_update )
  WHEN NOT MATCHED BY SOURCE AND target.contact_source_system_nm='vast' THEN
  DELETE
;
