create or replace table `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_phone_stage` as
with combined_cust_phone as
(
select contact_adw_key,
day_phone_adw_key as phone_adw_key,
'day' as phone_typ_cd,
cast(last_upd_dt as datetime) as last_upd_dt
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
where day_phone_adw_key!='-1'
union distinct
select contact_adw_key,
night_phone_adw_key as phone_adw_key,
'night' as phone_typ_cd,
cast(last_upd_dt as datetime) as last_upd_dt
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
where night_phone_adw_key!='-1'
union distinct
select contact_adw_key,
other_phone_adw_key as phone_adw_key,
'other' as phone_typ_cd,
cast(last_upd_dt as datetime) as last_upd_dt
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
where other_phone_adw_key!='-1'
union distinct
select contact_adw_key,
bill_phone_adw_key as phone_adw_key,
'bill' as phone_typ_cd,
cast(last_upd_dt as datetime) as last_upd_dt
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
where bill_phone_adw_key!='-1'
union distinct
select contact_adw_key,
fax_phone_adw_key as phone_adw_key,
'fax' as phone_typ_cd,
cast(last_upd_dt as datetime) as last_upd_dt
from `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_stage`
where fax_phone_adw_key!='-1'
--pulling in previously loaded target phones
union distinct
select
target.contact_adw_key,
target.phone_adw_key,
case when target.phone_typ_cd like 'day%' then 'day'
     when target.phone_typ_cd like 'night%' then 'night'
	 when target.phone_typ_cd like 'other%' then 'other'
	 when target.phone_typ_cd like 'bill%' then 'bill'
	 when target.phone_typ_cd like 'fax%' then 'fax'
	 end as phone_typ_cd,
...
lag(phone_adw_key) over (partition by contact_adw_key, phone_typ_cd order by cast(last_upd_dt as datetime), phone_adw_key) as prev_phone,
coalesce(lead(cast(last_upd_dt as datetime)) over (partition by contact_adw_key, phone_typ_cd order by cast(last_upd_dt as datetime), phone_adw_key), datetime('9999-12-31'
) )as next_record_date
...
phone_adw_key,
phone_typ_cd,
sum(new_phone_tag) over (partition by contact_adw_key, phone_typ_cd order by effective_start_datetime, phone_adw_key) as grouping_column,
effective_start_datetime,
next_record_date
...
concat(phone_typ_cd, case when rn=1 then '' else concat('-',cast(rn as string)) end ) as phone_type,
effective_start_datetime,
...
using `{{ var.value.INTEGRATION_PROJECT }}.adw_work.vast_contact_phone_stage` source
...
when not matched by source and target.contact_source_system_nm='vast' then delete;
