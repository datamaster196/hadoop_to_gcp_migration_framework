CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.d3_contact_source_key_stage` AS
WITH
  normalized_source_key AS (
  SELECT
    contact_adw_key,
    'service_call_key' AS key_typ_nm,
    ...
    CONCAT(ifnull(CAST(comm_ctr_id AS string),
        ...
        '')) AS dedupe_check
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.d3_contact_stage`
  UNION ALL
  SELECT
    contact_adw_key,
  ...
  SELECT
    contact_adw_key,
    case 
        when key_typ_nm like 'service_call_key%' then 'service_call_key'
  ...
  SELECT
    contact_adw_key,
    'd3' AS contact_source_system_nm,
    key_typ_nm,
    effective_start_datetime,
...
using `{{ var.value.INTEGRATION_PROJECT }}.adw_work.d3_contact_source_key_stage` source
