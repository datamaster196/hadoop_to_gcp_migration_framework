...
ROW_NUMBER() OVER(PARTITION BY source.cdpolicylinetypecode,source.typeofbusinesscode ORDER BY  adw_lake_insert_datetime  DESC) AS dupe_check
FROM
`{{ var.value.INGESTION_PROJECT }}.epic.cdpolicylinetype` as source
 WHERE
  CAST(adw_lake_insert_datetime as DATETIME)  > (
  SELECT
