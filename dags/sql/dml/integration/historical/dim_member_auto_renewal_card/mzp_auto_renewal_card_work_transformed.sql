...
SELECT
`{{ var.value.INTEGRATION_PROJECT }}.udfs.hash_name`(coalesce(trim(autorenewal.cc_first_name),''),'',coalesce(trim(autorenewal.cc_last_name),''),'','') as nm_adw_key,
`{{ var.value.INTEGRATION_PROJECT }}.udfs.hash_address`(coalesce(trim(autorenewal.CC_STREET),''),'','',coalesce(trim(autorenewal.CC_City),''),coalesce(trim(autorenewal.CC_State),''),coalesce(trim(autorenewal.CC_Zip),''),'','') as address_adw_key,
mbr_source_arc_key ,
mbr_arc_status_dtm,
mbr_cc_typ_cd,
...
FROM
 `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_member_auto_renewal_card_work_source` autorenewal where autorenewal.dup_check=1
