CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_member_rider_work_type_2_hist` AS
WITH
  ...
  SELECT
    ...
    LAG(adw_row_hash) OVER (PARTITION BY mbr_rider_source_key ORDER BY last_upd_dt) AS prev_row_hash,
    coalesce(LEAD(last_upd_dt) OVER (PARTITION BY mbr_rider_source_key ORDER BY last_upd_dt),
  ...
  SELECT
    mbr_rider_source_key,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY mbr_rider_source_key ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column
