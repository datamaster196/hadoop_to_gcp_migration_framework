...
,
 ROW_NUMBER() OVER (PARTITION BY mbr_rider_source_key,last_upd_dt ORDER BY last_upd_dt DESC ) AS DUP_CHECK
FROM
 `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_member_rider_work_source` AS rider left  join
 `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_member` member on SAFE_CAST(rider.member_ky as INT64)=member.mbr_source_system_key and member.actv_ind='Y' left  join
 `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_membership` membership on SAFE_CAST(rider.membership_ky as INT64) =membership.mbrs_source_system_key and membership.actv_ind='Y' left  join
 `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_employee_role` employee_role on employee_role.emp_role_id=rider.agent_id and employee_role.actv_ind='Y' left join
 `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_member_auto_renewal_card` auto_renewal on auto_renewal.mbr_source_arc_key=CAST(rider.AUTORENEWAL_CARD_KY as INT64) and auto_renewal.actv_ind='Y' left  join
  (select product.* from `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_product` product join
                         `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_product_category` dim_product_category ON product_category_cd IN  ('RCC','MBR_FEES') and dim_product_category.product_category_adw_key =product.product_category_adw_key  ) product on rider.rider_comp_cd=product.product_sku_key and product.actv_ind='Y' left join
  (select distinct code, code_desc, ROW_NUMBER() OVER (PARTITION BY code ORDER BY cast(last_upd_dt as datetime) DESC ) AS DUP_CHECK from `{{ var.value.INGESTION_PROJECT }}.mzp.cx_codes` where code_type = 'CANRES') as codes
         on rider.mbr_rider_cancel_reason_cd = codes.code and codes.DUP_CHECK = 1
where rider.dupe_check=1