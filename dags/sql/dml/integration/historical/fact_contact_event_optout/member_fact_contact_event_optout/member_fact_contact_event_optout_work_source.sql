CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.member_fact_contact_event_optout_work_source` AS
  ------------------------------------------------------------------------
  --------------------------------Source Member------------------------------
	SELECT 
  contact_adw_key,
...
FROM (
  ...
  SELECT
	  ...
	  membership_code.MEMBERSHIP_KY as source_system_key,
    ...
    'Opt-out' AS event_cd,
    SAFE_CAST(membership_code.LAST_UPD_DT AS string) AS event_dtm,
    'membership' AS src
  FROM
    `{{ var.value.INGESTION_PROJECT }}.mzp.membership_code` membership_code
  LEFT JOIN
      ( select distinct
        ...
        join (select mbrs_adw_key, contact_adw_key,mbr_source_system_key, effective_start_datetime, effective_end_datetime from `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_member` where actv_ind = 'Y') mem
			...
			on safe_cast(membership_code.MEMBERSHIP_KY as INT64) = contact.source_1_key
	LEFT JOIN
  ( select distinct
      mbrship.mbrs_source_system_key as source_1_key, 
			mem.contact_adw_key,
	...
	ON safe_cast(membership_code.MEMBERSHIP_KY  as INT64) = contact_N_eff.source_1_key and SAFE_CAST(SAFE_CAST(membership_code.LAST_UPD_DT AS string) as DATETIME) between contact_N_eff.effective_start_datetime and contact_N_eff.effective_end_datetime
  WHERE
    membership_code.code IN( 'DNDM','DNE','FSC','INS','NMAG','NMSP','ORP','TRV','STOP')
  ...
  SELECT
	  ...
	  member_code.member_ky as source_system_key,
    'MZP' AS system_of_record,
    '' AS topic_nm,
    (CASE
    ...
    'Opt-out' AS event_cd,
    SAFE_CAST(member_code.LAST_UPD_DT AS string) AS event_dtm,
    'member' AS src
  FROM
    `{{ var.value.INGESTION_PROJECT }}.mzp.member_code` member_code
  LEFT JOIN
  ( select 
     contact_adw_key, 
     source_1_key, 
  ...
  ON member_code.member_ky=con_member.source_1_key and con_member.rn=1
  LEFT JOIN
	    `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_contact_source_key` con_member_N_eff
	ON con_member_N_eff.key_typ_nm LIKE 'member_key%' AND member_code.member_ky=con_member_N_eff.source_1_key
	    AND SAFE_CAST(SAFE_CAST(member_code.LAST_UPD_DT AS string) as DATETIME) between con_member_N_eff.effective_start_datetime and con_member_N_eff.effective_end_datetime
  WHERE
    member_code.code IN( 'DNC','DNE','NOE','DNM','DNT') 
	) mbrcode_contact where 	mbrcode_contact.contact_adw_key <> '-1'
) MZP
) FINAL where dupe_check = 1