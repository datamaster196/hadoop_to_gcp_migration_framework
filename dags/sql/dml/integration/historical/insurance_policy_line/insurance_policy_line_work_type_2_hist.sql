CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.insurance_policy_line_work_type_2_hist` AS
WITH
  policy_line_hist AS (
  SELECT
    ...
    LAG(adw_row_hash) OVER (PARTITION BY ins_policy_line_source_key ORDER BY effective_start_datetime) AS prev_row_hash,
    coalesce(LEAD(cast(effective_start_datetime as datetime)) OVER (PARTITION BY ins_policy_line_source_key ORDER BY effective_start_datetime),
  ...
  SELECT
    ins_policy_line_source_key,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY ins_policy_line_source_key ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column 
