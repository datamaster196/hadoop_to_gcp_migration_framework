CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_membership_work_type_2_hist` AS
WITH
  membership_hist AS (
  SELECT
    ...
    LAG(adw_row_hash) OVER (PARTITION BY mbrs_source_system_key ORDER BY last_upd_dt) AS prev_row_hash,
    coalesce(LEAD(last_upd_dt) OVER (PARTITION BY mbrs_source_system_key ORDER BY last_upd_dt),
      datetime('9999-12-31' ) )AS next_record_datetime
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.mzp_membership_work_transformed`
  ...
  SELECT
    mbrs_source_system_key,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY mbrs_source_system_key ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column
