CREATE OR REPLACE TABLE
  ...
  company_source.nameof as vendor_name,
   ...
   CASE WHEN contactname.fullname IS NULL OR contactname.fullname=''
    ...
    CASE WHEN SUBSTR(Nameof,0,4) = 'CSAA'
    THEN 'Preferred'
    ELSE 'Non-Preferred'
    ...
    CASE WHEN company_source.updateddate IS NULL
    	 THEN company_source.ts
       ELSE company_source.updateddate
       END as updateddate,
  ...
  coalesce(TRIM(contactaddress.cdcountrycode),
    '') AS cdcountrycode,
  ROW_NUMBER() OVER(PARTITION BY company_source.lookupcode , company_source.updateddate ORDER BY NULL DESC) AS DUPE_CHECK
  FROM
