CREATE OR REPLACE TABLE
  `{{var.value.INTEGRATION_PROJECT}}.adw_work.insurance_product_type_2_hist` AS
WITH
  product_hist AS (
  SELECT
    ...
    LAG(adw_row_hash) OVER (PARTITION BY product_sku,product_category_adw_key ORDER BY last_upd_dt) AS prev_row_hash,
    coalesce(LEAD(last_upd_dt) OVER (PARTITION BY product_sku,product_category_adw_key ORDER BY last_upd_dt),
  ...
  SELECT
    product_sku,product_category_adw_key,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY product_sku,product_category_adw_key ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column
