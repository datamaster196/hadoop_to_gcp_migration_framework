...
solicitation.mbrs_solicit_adw_key,
 role.emp_role_adw_key as emp_role_adw_key ,
...
SELECT
    ...
    SOLICITATION_CD,
    SOURCE_OF_SALE,
    ROW_NUMBER() OVER (PARTITION BY SOLICITATION_CD,SOURCE_OF_SALE ORDER BY LAST_UPD_DT DESC ) DUPE_CHECK
FROM `{{ var.value.INGESTION_PROJECT }}.mzp.solicitation`) solic
on member.solicitation_cd = solic.SOLICITATION_CD
    AND member.source_of_sale = solic.SOURCE_OF_SALE
    AND solic.DUPE_CHECK=1
LEFT OUTER JOIN `{{ var.value.INTEGRATION_PROJECT }}.adw.dim_membership_solicitation` solicitation
ON (SAFE_CAST(solicitation.mbrs_solicit_source_system_key AS STRING)=solic.SOLICITATION_KY
    and solicitation.actv_ind='Y'
    )
LEFT JOIN
(
SELECT
