...
SAFE_CAST(source.total_tax AS NUMERIC),
 SAFE_CAST(hist_type_2.effective_start_datetime AS datetime),
  ...
  {{ dag_run.id }}
FROM
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.fact_sales_header_pos_work_type_2_hist` hist_type_2
JOIN
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.fact_sales_header_pos_work_transformed` source ON (
  CONCAT('POS',source.ofc_id,source.sale_id) = hist_type_2.fact_sales_header_source_key
  AND source.effective_start_datetime=hist_type_2.effective_start_datetime)
LEFT JOIN (
  SELECT
	GENERATE_UUID() AS sales_header_adw_key,
    CONCAT('POS', ofc_id , sale_id )  fact_sales_header_source_key
  FROM
    `{{ var.value.INTEGRATION_PROJECT }}.adw_work.fact_sales_header_pos_work_transformed`
  GROUP BY
    ofc_id,
    sale_id) pk
ON (CONCAT('POS',source.ofc_id,source.sale_id) =pk.fact_sales_header_source_key);
...
HAVING
 ...
 select count(1)
   from
   (select source_system,source_system_key , aca_office_adw_key, effective_start_datetime, count(*) as dupe_count 
