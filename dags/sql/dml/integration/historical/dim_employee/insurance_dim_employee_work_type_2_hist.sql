CREATE OR REPLACE TABLE
  `{{ var.value.INTEGRATION_PROJECT }}.adw_work.insurance_employee_type_2_hist` AS
WITH
  employee_hist AS (
  SELECT
    ...
    LAG(adw_row_hash) OVER (PARTITION BY emp_active_directory_user_id ORDER BY last_upd_dt) AS prev_row_hash,
    coalesce(LEAD(last_upd_dt) OVER (PARTITION BY emp_active_directory_user_id ORDER BY last_upd_dt),
  ...
  SELECT
    emp_active_directory_user_id,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY emp_active_directory_user_id ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column ),
