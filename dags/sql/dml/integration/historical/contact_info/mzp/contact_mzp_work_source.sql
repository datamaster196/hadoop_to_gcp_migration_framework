...
SELECT
  ...
  coalesce(greatest(cast(member.last_upd_dt as datetime), cast(membership.last_upd_dt as datetime)), greatest(cast(member.status_dt as datetime), cast(membership.status_dt as datetime))) as last_upd_dt,
...
from (select
      memb.*,
       coalesce(lead(cast(memb.last_upd_dt as  datetime)) over (partition by memb.member_ky order by memb.last_upd_dt), datetime('9999-12-31')) as eff_end_dt,
...
join (select
      *,
      ...
      from `{{ var.value.INGESTION_PROJECT }}.mzp.membership`) membership on member.membership_ky=membership.membership_ky and  cast(member.last_upd_dt as datetime)<=membership.eff_end_dt and cast(membership.last_upd_dt as datetime)<=member.eff_end_dt
left join (select
      *,
      row_number() over (partition by branch_ky order by last_upd_dt desc) as rn
      from `{{ var.value.INGESTION_PROJECT }}.mzp.branch`) branch on membership.branch_ky=branch.branch_ky and branch.rn=1
) mzp
