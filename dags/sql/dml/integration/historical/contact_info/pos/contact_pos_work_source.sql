...
GREATEST(coalesce(cust.effective_start_datetime, datetime('2000-01-01')), 
           coalesce(cust_adr.effective_start_datetime, datetime('2000-01-01')), 
		   coalesce(cust_tel.effective_start_datetime, datetime('2000-01-01')),
...
from ( SELECT
    *,
	cast(last_update as datetime) as effective_start_datetime,
    coalesce(LEAD(cast(last_update as datetime)) OVER (PARTITION BY cust_id ORDER BY last_update ASC), cast('9999-12-31' as datetime)) as effective_end_datetime 
  FROM `{{ var.value.INGESTION_PROJECT }}.pos.cust` 
  ) cust
left outer join ( SELECT
    *,
	cast(last_update as datetime) as effective_start_datetime,
    coalesce(LEAD(cast(last_update as datetime)) OVER (PARTITION BY cust_id, cust_adr_typ_id ORDER BY last_update ASC), cast('9999-12-31' as datetime)) as effective_end_datetime 
  FROM `{{ var.value.INGESTION_PROJECT }}.pos.cust_adr` 
  ) cust_adr
on cust.cust_id=cust_adr.cust_id
   AND cust.effective_end_datetime>=cust_adr.effective_start_datetime
   AND cust.effective_start_datetime<cust_adr.effective_end_datetime
left outer join ( SELECT
    *,
	cast(last_update as datetime) as effective_start_datetime,
    coalesce(LEAD(cast(last_update as datetime)) OVER (PARTITION BY cust_id, cust_tel_typ_id ORDER BY last_update ASC), cast('9999-12-31' as datetime)) as effective_end_datetime
  FROM `{{ var.value.INGESTION_PROJECT }}.pos.cust_tel`
  where length(trim(cust_tel_nr))>1 
  ) cust_tel
on cust.cust_id=cust_tel.cust_id
   AND cust.effective_end_datetime>=cust_tel.effective_start_datetime
   AND cust.effective_start_datetime<cust_tel.effective_end_datetime
) pos 
