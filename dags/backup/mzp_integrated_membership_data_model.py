...
SAFE_CAST(TIMESTAMP(source.member_rider_cancel_date) as DATE) member_rider_cancel_date,
SAFE_CAST(TIMESTAMP(source.member_rider_cost_effective_date) as DATE) member_rider_cost_effective_date,
...
SAFE_CAST(TIMESTAMP(source.member_rider_future_cancel_date) as DATE) member_rider_future_cancel_date,
source.member_rider_billing_category_code,
source.member_rider_cancel_reason_code,
source.member_rider_reinstate_indicator,
SAFE_CAST(TIMESTAMP(source.member_rider_effective_date) as DATE) member_rider_effective_date,
...
SAFE_CAST(TIMESTAMP(source.member_rider_activation_date) as DATE) member_rider_activation_date,
last_upd_dt AS effective_start_datetime,
    CAST('9999-12-31' AS datetime) effective_end_datetime,
    'Y' AS active_indicator,
    source.adw_row_hash,
...
WITH employee_lookup AS (
  SELECT
    ...
    bb.employee_adw_key
  FROM
    `{iu.INGESTION_PROJECT}.mzp.cx_iusers` aa
  LEFT JOIN (
    SELECT
      employee_adw_key,
...
SAFE_CAST(TIMESTAMP(source.member_cc_expiration_date) as DATE) member_cc_expiration_date,
source.member_cc_reject_reason_code,
SAFE_CAST(TIMESTAMP(source.member_cc_reject_date) as DATE) member_cc_reject_date,
...
source.AUTO_RENEW_FL payment_received_auto_renewal_flag ,
  ...
  ON
    (source.batch_payment_ky=SAFE_CAST(target.payment_received_source_key AS STRING)
      AND target.actrive_indicator='Y')
  WHERE
...
b.integrate_update_batch_number)
  ...
  SET
    a.effective_end_datetime = b.effective_end_datetime,
    a.actrive_indicator = b.active_indicator,
    a.integrate_update_datetime = b.integrate_update_datetime,
    a.integrate_update_batch_number = b.integrate_update_batch_number
"""
