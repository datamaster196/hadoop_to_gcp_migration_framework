...
DAG_TITLE = "integrated_contact_epic_hist"
...
from 
     (select 
      *,
	  cast(substr(updateddate, 1, 23) as datetime) as effective_start_date,
	  cast(coalesce(lead(substr(updateddate, 1, 23)) over (partition by uniqentity order by updateddate asc),'9999-12-31') as datetime) as  effective_end_date
      from `adw-lake-dev.epic.client` where uniqentity<>'-1') client 
left join (select 
      *,
	  cast(substr(updateddate, 1, 23) as datetime) as effective_start_date,
	  cast(coalesce(lead(substr(updateddate, 1, 23)) over (partition by uniqentity order by updateddate asc),'9999-12-31') as datetime) as  effective_end_date	  
      from `adw-lake-dev.epic.contactname`) contactname on client.uniqcontactnameprimary=contactname.uniqcontactname and client.effective_end_date>=contactname.effective_start_date and client.effective_start_date<contactname.effective_end_date
left join (select 
      *,
	  cast(substr(updateddate, 1, 23) as datetime) as effective_start_date,
	  cast(coalesce(lead(substr(updateddate, 1, 23)) over (partition by uniqentity order by updateddate asc),'9999-12-31') as datetime) as  effective_end_date
      from `adw-lake-dev.epic.contactaddress`) contactaddress on client.uniqcontactaddressaccount=contactaddress.uniqcontactaddress and client.effective_end_date>=contactaddress.effective_start_date and client.effective_start_date<contactaddress.effective_end_date
left join (select 
      *,
	  cast(substr(updateddate, 1, 23) as datetime) as effective_start_date,
	  cast(coalesce(lead(substr(updateddate, 1, 23)) over (partition by uniqentity order by updateddate asc),'9999-12-31') as datetime) as  effective_end_date
      from `adw-lake-dev.epic.contactnumber`) contactnumber on client.uniqcontactnumberaccount=contactnumber.uniqcontactnumber and client.effective_end_date>=contactnumber.effective_start_date and client.effective_start_date<contactnumber.effective_end_date
