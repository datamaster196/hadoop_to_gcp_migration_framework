...
DAG_TITLE = "integrated_contact_d3_hist"
...
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(calr_bsc_ad)))                                                    as cleansed_address_1_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(calr_supl_ad)))                                                   as cleansed_address_2_name,
cast(null as string)                                                      as cleansed_care_of_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(calr_cty_nm)))                                                    as cleansed_city_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(calr_st_cd)))                                                     as cleansed_state_code,
...
from (select distinct 
			...
			where membership_id != '0000000' and coalesce(membership_id, '') != ''
            )
),
matched_guid as
(
