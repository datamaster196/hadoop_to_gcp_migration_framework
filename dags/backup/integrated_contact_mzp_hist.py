...
DAG_TITLE = "integrated_contact_mzp_hist"
...
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(membership.address_line1)))                                  as cleansed_address_1_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(membership.address_line2)))                                  as cleansed_address_2_name,
cast(null as string)                                                 as cleansed_care_of_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(membership.city)))                                           as cleansed_city_name,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(membership.state)))                                          as cleansed_state_code,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    trim(membership.zip))                                            as cleansed_postal_code,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    trim(membership.delivery_route))                                 as cleansed_postal_plus_4_code,
`{int_u.INTEGRATION_PROJECT}.udfs.udf_remove_selected_punctuation`(
    lower(trim(membership.country)))                                        as cleansed_country_name,
coalesce(lower(trim(member.email)),'')                                      as cleansed_email_name,
...
join (select 
      *, 
      ...
      from `{iu.INGESTION_PROJECT}.mzp.membership`) membership on member.membership_ky=membership.membership_ky and  cast(member.last_upd_dt as datetime)<=membership.eff_end_dt and cast(membership.last_upd_dt as datetime)<=member.eff_end_dt
join (select 
      *, 
      row_number() over (partition by branch_ky order by last_upd_dt desc) as rn
      from `{iu.INGESTION_PROJECT}.mzp.branch`) branch on membership.branch_ky=branch.branch_ky and branch.rn=1
...
'0' as target_match_on_member_id_indicator,
...
'0' as source_match_on_member_id_indicator,
...
source.member_ky as source_system_key,
address_match.contact_adw_key as contact_adw_key
from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_contact_unmatched_work_1` source    
      -- Matching Criteria 2: First Name, Last Name, Address Line 1, City, State, Name Suffix --
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      , cleansed_state_code
	  , mk1.member_ky
      from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_contact_unmatched_work_1` mk1
      join `{int_u.INTEGRATION_PROJECT}.adw.dim_contact_source_key` cs1 
      on mk1.member_ky=cs1.source_1_key and 
      cs1.contact_source_system_name='mzp' and
      cs1.key_type_name='member_key'
      group by 
      cleansed_first_name
      , cleansed_last_name
      , cleansed_suffix_name                      
      ...
      and source.cleansed_city_name       = address_match.cleansed_city_name         
      and source.cleansed_state_code      = address_match.cleansed_state_code
	    and source.member_ky                != address_match.member_ky	  
      and source.cleansed_first_name      != ''                          
      and source.cleansed_last_name       != ''                                    
      and source.cleansed_address_1_name  != ''                           
      and source.cleansed_city_name       != ''                          
...
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      and source.cleansed_address_1_name   = zip5_match.cleansed_address_1_name
      and source.cleansed_postal_code      = zip5_match.cleansed_postal_code
	    and source.member_ky                != zip5_match.member_ky
      and source.cleansed_first_name      != ''
      and source.cleansed_last_name       != ''                 
      and source.cleansed_address_1_name  != ''
      and source.cleansed_postal_code     != ''
...
join (select max(cs1.contact_adw_key) as contact_adw_key
      ...
      and source.cleansed_suffix_name      = email_match.cleansed_suffix_name                     
      and source.cleansed_email_name       = email_match.cleansed_email_name
	  and source.member_ky                != email_match.member_ky
      and source.cleansed_first_name      != ''
      and source.cleansed_last_name       != ''             
      and source.cleansed_email_name      != ''
) a
...
with DAG(DAG_TITLE, default_args=default_args, schedule_interval=None) as dag:
    ...
    task_contact_insert >> task_contact_merge >> task_contact_audit
