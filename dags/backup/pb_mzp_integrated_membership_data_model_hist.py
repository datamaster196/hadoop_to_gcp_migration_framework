...
DAG_TITLE = "pb_integrated_membership_payment_applied_detail_hist"
...
FROM
 ...
 on member.member_ky =SAFE_CAST(adw_memship.membership_source_system_key AS STRING) AND active_indicator='Y'
left join
...
on solicitation.solicitation_code=member.solicitation_cd and solicitation.active_indicator='Y'
...
SAFE_CAST(member_rider_cancel_date as DATE),	
SAFE_CAST(member_rider_cost_effective_date as DATE),	
member_rider_solicitation_code,	
member_rider_do_not_renew_indicator,	
...
member_rider_paid_by_code,	
SAFE_CAST(member_rider_future_cancel_date as DATE),	
member_rider_billing_category_code,	
member_rider_cancel_reason_code,	
member_rider_reinstate_indicator,	
SAFE_CAST(member_rider_effective_date as DATE),	
...
SAFE_CAST(member_rider_activation_date as DATE),	
CAST(hist_type_2.effective_start_datetime AS datetime),
CAST(hist_type_2.effective_end_datetime AS datetime),
...
SELECT 
 ...
 ,membership_exp_dt as member_expiration_dt
...
WITH
  ...
  SELECT
    ...
    adw_row_hash,
    effective_start_datetime,
    LAG(effective_start_datetime) OVER (PARTITION BY segment_key ORDER BY effective_start_datetime) AS prev_row_hash,
    coalesce(LEAD(effective_start_datetime) OVER (PARTITION BY segment_key ORDER BY effective_start_datetime),
      datetime('9999-12-31' ) )AS next_record_datetime
  FROM
  ...
  SELECT
    segment_key,
    adw_row_hash,
    SUM(new_record_tag) OVER (PARTITION BY segment_key ORDER BY effective_start_datetime) AS grouping_column,
    effective_start_datetime,
    next_record_datetime
  FROM
    set_grouping_column 
...
SELECT
  membership_marketing_segmentation_adw_key
  ,source.membership_adw_key
  ,SAFE_CAST(source.member_expiration_dt AS date)
...
JOIN
  ...
  source.segment_key=hist_type_2.segment_key
...
LEFT JOIN (
  ...
  GROUP BY
    segment_key) pk
ON (source.segment_key=pk.segment_key)
...
member_cc_type_code			 ,
SAFE_CAST(member_cc_expiration_date as DATE)	, 
member_cc_reject_reason_code,
SAFE_CAST(member_cc_reject_date as DATE) ,
member_cc_donor_number		, 
member_cc_last_four			 ,
member_credit_debit_type_code,		
...
source.rev_membership_payment_ky=SAFE_CAST(payments_received.payment_received_batch_key as STRING) and payments_received.actrive_indicator='Y' )
LEFT JOIN
...
with DAG(DAG_TITLE, default_args=default_args, schedule_interval=None) as dag:
    ...
    # membership_payment_applied_detail
    task_member_pay_app_detail_work_source >> task_member_pay_app_detail_work_transformed >> task_member_pay_app_detail_work_type2_logic >> task_membership_pay_app_detail_insert
