...
DAG_TITLE = "integrated_customer_mzp"
...
######################################
customer_work_source = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_source_work` as
      select
          membership.membership_ky                                              as membership_ky,
          member.member_ky                                                      as member_ky,
          438                                                                   as iso_cd,
          branch.club_cd                                                        as club_cd,
          member.membership_id                                                  as membership_id,
          member.associate_id                                                   as associate_id,
          member.check_digit_nr                                                 as check_digit_nr,
          ...
          coalesce(least(cast(member.last_upd_dt as datetime)
                  ,cast(membership.last_upd_dt as datetime))
                  ,least(cast(member.status_dt as datetime)
                  ,cast(membership.status_dt as datetime)))                     as last_upd_dt,
      ...
      from 
          (select 
            *, 
             coalesce(lead(cast(last_upd_dt as  datetime)) over 
                 (partition by member_ky order by last_upd_dt), 
                    datetime('9999-12-31'))                          as eff_end_dt
            from `{iu.INGESTION_PROJECT}.mzp.member`) member
          join (select 
                       *, 
                       coalesce(lead(cast(last_upd_dt as  datetime)) over 
                           (partition by membership_ky order by last_upd_dt), 
                              datetime('9999-12-31'))                          as eff_end_dt
                  from `{iu.INGESTION_PROJECT}.mzp.membership`) membership 
                 on member.membership_ky=membership.membership_ky 
                and  cast(member.last_upd_dt as datetime)<=membership.eff_end_dt 
                and cast(membership.last_upd_dt as datetime)<=member.eff_end_dt
          join (select 
                      *, 
               ...
               and branch.rn = 1
"""
customer_work_target = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_target_work` as 
      select 
          customer.customer_adw_key           as customer_adw_key,
          member_key.customer_source_1_key    as member_ky,
          name.name_first_name                as first_name,
          name.name_middle_name               as middle_name,
          name.name_last_name                 as last_name,
          name.name_suffix_name               as name_suffix,
          name.name_title_name                as salutation,
          address.address_1_name              as address_line1,
          address.address_city_name           as city,
          address.address_state_code          as state,
          address.address_postal_code         as zip,
          address.address_postal_plus_4_code  as address_postal_plus_4_code ,
          address.address_country_name        as country,
          email.email_name                    as email,
          customer.customer_birth_date        as birthdate,
      ...
      join `{int_u.INTEGRATION_PROJECT}.adw.dim_customer_source_key` member_key 
      	...
      	and member_key.customer_source_system_name     = 'mzp' and 
      	and member_key.customer_key_type_name          = 'member_key' and
      	and member_key.effective_end_datetime              = date('9999-12-31')	
      
      join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_name` name_bridge 
           on customer.customer_adw_key                  = name_bridge.customer_adw_key
      	and name_bridge.customer_source_system_name    = 'mzp'
      	and name_bridge.name_type_code                 = 'membership'
      	and name_bridge.effective_end_datetime             = date('9999-12-31')
      ...
      join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_address` address_bridge 
           on customer.customer_adw_key                  = address_bridge.customer_adw_key
      	and address_bridge.customer_source_system_name = 'mzp'
      	and address_bridge.address_type_code           = 'membership'
      	and address_bridge.effective_end_datetime          = date('9999-12-31')
      ...
      left join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_email` email_bridge 
          on customer.customer_adw_key                   = email_bridge.customer_adw_key
         and email_bridge.customer_source_system_name    = 'mzp'
         and email_bridge.email_type_code                = 'membership'
         and email_bridge.effective_end_datetime             = date('9999-12-31')
...
customer_work_matched = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_matched` as
      select
          ...
          source.cleansed_name_suffix,                      
          source.cleansed_address_line1,
          source.cleansed_city,
          source.cleansed_state,
          source.cleansed_zip,
          source.cleansed_email                 
...
customer_work_unmatched_1 = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_unmatched_work_1` as
      with member_guid as
          (select
                 GENERATE_UUID() as customer_adw_key,
                 member_ky
             from (select distinct member_ky 
                     from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_matched` 
                    where customer_adw_key is null
                   )
          ) 
      select
      ...
      from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_matched` original
          left outer join member_guid b 
             on original.member_ky=b.member_ky
...
customer_work_unmatched_2 = f"""
       ...
       from   `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_unmatched_work_1` original
              left outer join matched_guid matched 
                   on original.customer_adw_key = matched.orig_customer_adw_key
...
customer_work_final_staging = f"""
    ...
    select 
      ...
      check_digit_nr,
      customer_id,
      `{int_u.INTEGRATION_PROJECT}.udfs.hash_name`
           (first_name, middle_name, last_name, name_suffix, salutation) as name_adw_key,
      first_name,
      middle_name,
      last_name,
      name_suffix,
      salutation,
      `{int_u.INTEGRATION_PROJECT}.udfs.hash_address`
           (address_line1, address_line2, city, state, zip, country) as address_adw_key,
      address_line1,
      address_line2,
      city,
      state,
      zip,
      country,
      `{int_u.INTEGRATION_PROJECT}.udfs.hash_phone`
          (phone) as phone_adw_key,
      phone,
      `{int_u.INTEGRATION_PROJECT}.udfs.hash_email`
          (email) as email_adw_key,
      email,
      gender,
      birthdate,
      last_upd_dt,
...
query_name_insert = f"""insert into `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_name`
   ...
   where name_adw_key not in 
         (select distinct name_adw_key 
            from `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_name`)
"""

query_name_bridge = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_name_stage` as
     ...
     (
        select
              ...
              lag(name_adw_key) over 
                  (partition by customer_adw_key order by cast(last_upd_dt as date)) as prev_name,
              coalesce(lead(cast(last_upd_dt as date)) over 
                  (partition by customer_adw_key order by cast(last_upd_dt as date)), date('9999-12-31'))as next_record_date
    ...
    (
     ...
     (
       select
            ...
            row_number() over
               (partition by customer_adw_key,  effective_start_datetime 
                    order by effective_end_datetime desc,name_adw_key ) as rn
       from deduped
     )
...
query_name_merge = f"""merge into `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_name` target
...
query_address_insert = f"""insert into `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_address`
     ...
     select
         ...
         1 as batch
     from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_stage`
     where address_adw_key not in (select distinct address_adw_key 
                                     from `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_address`)
"""
...
query_address_bridge = f"""
     ...
     (
         select
               customer_adw_key,
               address_adw_key,
               sum(new_address_tag) over
                  (partition by customer_adw_key order by effective_start_datetime) as grouping_column,
               effective_start_datetime,
               next_record_date
         from set_grouping_column
     ), 
     ...
     (
         select
              ...
              effective_end_datetime,
              row_number() over 
                 (partition by customer_adw_key,  effective_start_datetime order by effective_end_datetime desc,address_adw_key ) as rn
         from deduped
     )
     ...
     select
           ...
           address_adw_key,
           'mzp' as source_system,
           concat('membership', case when rn=1 then '' 
                                     else concat('-',cast(rn as string)) 
                                 end ) as address_type,
           effective_start_datetime,
           effective_end_datetime,
...
query_address_merge = f"""merge into `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_address` target
...
# PHONE
query_phone_insert = f"""insert into `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_phone`
      (
          phone_adw_key ,    
          phone_number,
      ...
      select
          ...
          1 as batch
      from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_stage`
      where phone_adw_key not in (select distinct phone_adw_key 
                                    from `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_phone`)
...
query_phone_bridge = f"""
      ...
      (select
            ...
            phone_adw_key,
            cast(last_upd_dt as date) as effective_start_datetime,
            lag(phone_adw_key) over 
                (partition by customer_adw_key order by cast(last_upd_dt as date)) as prev_phone,
            coalesce(lead(cast(last_upd_dt as date)) over 
                (partition by customer_adw_key order by cast(last_upd_dt as date)), date('9999-12-31'
            ) )as next_record_date
      ...
      select
            ...
            effective_end_datetime,
            row_number() over
               (partition by customer_adw_key,  effective_start_datetime order by effective_end_datetime desc,phone_adw_key ) as rn
      from deduped
      )
...
# EMAIL
query_email_insert = f"""insert into `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_email`
      (
         email_adw_key,
         email_name,    
         integrate_insert_datetime,
...
query_email_bridge = f"""create or replace table `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_email_stage` as
      ...
      (
        select
          customer_adw_key,
          email_adw_key,
          case when prev_email is null or prev_email<>email_adw_key then 1 
               else 0 
           end as new_email_tag,
          effective_start_datetime,
          next_record_date
        from cust_email
        ), 
      ...
      (
        select
          customer_adw_key,
          email_adw_key,
          sum(new_email_tag) over 
             (partition by customer_adw_key order by effective_start_datetime, email_adw_key) as grouping_column,
          effective_start_datetime,
          next_record_date
        from set_grouping_column
      ), 
      ...
      (
        select
           ...
           effective_end_datetime,
           row_number() over
              (partition by customer_adw_key,  effective_start_datetime order by effective_end_datetime desc,email_adw_key ) as rn
        from deduped
      )
...
query_email_merge = f"""merge into `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_email` target
...
# KEYS
query_source_key = f"""CREATE OR REPLACE TABLE
...
"""
query_source_key_merge = f"""merge into `{int_u.INTEGRATION_PROJECT}.adw.dim_customer_source_key` target
      using `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_source_key_stage` source
          on target.customer_adw_key            = source.customer_adw_key
         and target.customer_source_system_name = source.customer_source_system_name
         and target.customer_key_type_name      = source.customer_key_type_name
         and target.effective_start_datetime        = source.effective_start_datetime
      when matched and target.effective_end_datetime=cast('9999-12-31' as date) 
           then update
      set 
         target.effective_end_datetime=source.effective_end_datetime,
         target.integrate_update_datetime=source.update_datetime,
...
# CUSTOMER
query_customer_insert = f"""insert into `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_customer`
      (
        customer_adw_key ,    
      ...
      select
        ...
        1 as batch_update
      from `{int_u.INTEGRATION_PROJECT}.adw_work.mzp_customer_stage`
      where customer_adw_key not in (select distinct customer_adw_key 
                                       from `{int_u.INTEGRATION_PROJECT}.adw_pii.dim_customer`)
      ...
      (
        ...
        (
          select
             customer_adw_key,
             customer_source_system_name,
             row_number() over(partition by customer_adw_key order by case when customer_source_system_name='mzp' then 1 
                                                                           when customer_source_system_name='epic' then 2 
                                                                           when customer_source_system_name='d3' then 3 
                                                                       end asc, effective_end_datetime desc, effective_start_datetime desc) as rn
          from `{int_u.INTEGRATION_PROJECT}.adw.dim_customer_source_key`
        ) primacy 
          on customer.customer_adw_key = primacy.customer_adw_key	
         and primacy.rn=1
        
        left outer join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_name` name_bridge 
            on customer.customer_adw_key = name_bridge.customer_adw_key
        	 and name_bridge.customer_source_system_name = primacy.customer_source_system_name
        	 and name_bridge.name_type_code = 'membership'
        	 and name_bridge.effective_end_datetime = date('9999-12-31')
        ...
        left outer join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_address` address_bridge 
            on customer.customer_adw_key = address_bridge.customer_adw_key
        	 and address_bridge.customer_source_system_name = primacy.customer_source_system_name
        	 and address_bridge.address_type_code = 'membership'
        	 and address_bridge.effective_end_datetime = date('9999-12-31')
        ...
        left outer join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_email` email_bridge 
            on customer.customer_adw_key=email_bridge.customer_adw_key
        	 and email_bridge.customer_source_system_name=primacy.customer_source_system_name
        	 and email_bridge.email_type_code='membership'
        	 and email_bridge.effective_end_datetime=date('9999-12-31')
        ...
        left outer join `{int_u.INTEGRATION_PROJECT}.adw.xref_customer_phone` phone_bridge 
            on customer.customer_adw_key=phone_bridge.customer_adw_key
        	 and phone_bridge.customer_source_system_name=primacy.customer_source_system_name
        	 and phone_bridge.phone_type_code='membership'
        	 and phone_bridge.effective_end_datetime=date('9999-12-31')
