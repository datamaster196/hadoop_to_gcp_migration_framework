#######################################################################################################################
##  DAG_NAME :  integrated_membership_mzp
##  PURPOSE :   Executes incremental load for conatct info and membership (mzp) model
##  PREREQUISITE DAGs :
#######################################################################################################################
...
DAG_TITLE = "integrated_membership_mzp"
SCHEDULE = Variable.get("integrated_membership_mzp_schedule", default_var='') or None

SENSOR_TIMEDELTA_MZP = Variable.get("integrated_membership_mzp_timedelta", default_var='') or None
...
with DAG(DAG_TITLE, default_args=default_args, schedule_interval=SCHEDULE, catchup=False,
   ...
   ######################################################################
    ...
    sensor_ingest_mzp_task = ExternalTaskSensor(task_id='sensor_ingest_mzp',
                                               external_dag_id='ingest_mzp',
                                               external_task_id='AllTaskSuccess',
                                               #execution_delta = timedelta(minutes=[SENSOR_TIMEDELTA_MZP]),
...
#    sensor_ingest_sigma_task = ExternalTaskSensor(task_id='sensor_ingest_sigma_add_assoc_file',
#                                            external_dag_id='ingest_sigma_add_assoc_file',
#                                            external_task_id='AllTaskSuccess',
#                                            #execution_delta = timedelta(minutes=[SENSOR_TIMEDELTA_MZP]),
#                                            execution_delta=timedelta(minutes=170),
#                                            AirflowSensorTimeout=10 * 60,
#                                            check_existance=True,
#                                            dag=dag
#                                            )
    ...
    task_contact_work_source = int_u.run_query("build_contact_work_source","dml/integration/incremental/contact_info/mzp/contact_mzp_work_source.sql")
    ...
    # Product
    task_product_source = int_u.run_query('build_product_source',
                                          "dml/integration/incremental/dim_product/mzp_product_work_source.sql")
    task_product_transformed = int_u.run_query('product_transformed',
                                               "dml/integration/incremental/dim_product/mzp_product_work_transformed.sql")
    task_product_stage = int_u.run_query('product_stage',
                                         "dml/integration/incremental/dim_product/mzp_product_work_stage.sql")
    task_product_merge = int_u.run_query('product_merge',
                                         "dml/integration/incremental/dim_product/dim_product.sql")

    # Membership

    # Membership - Workarea loads
    task_membership_work_source = int_u.run_query("build_membership_work_source",
                                                  "dml/integration/incremental/dim_membership/mzp_membership_work_source.sql")
    task_membership_work_transformed = int_u.run_query("build_membership_work_transformed",
                                                       "dml/integration/incremental/dim_membership/mzp_membership_work_transformed.sql")
    task_membership_work_final_staging = int_u.run_query("build_membership_work_final_staging",
                                                         "dml/integration/incremental/dim_membership/mzp_membership_work_stage.sql")
    task_membership_merge = int_u.run_query("merge_membership",
                                            "dml/integration/incremental/dim_membership/dim_membership.sql")
    ...
    task_member_work_source = int_u.run_query("build_member_work_source",
                                              "dml/integration/incremental/dim_member/mzp_member_work_source.sql")
    task_member_work_transformed = int_u.run_query("build_member_work_transformed",
                                                   "dml/integration/incremental/dim_member/mzp_member_work_transformed.sql")
    task_member_work_final_staging = int_u.run_query("build_member_work_final_staging",
                                                     "dml/integration/incremental/dim_member/mzp_member_work_stage.sql")
    task_member_merge = int_u.run_query('merge_member', "dml/integration/incremental/dim_member/dim_member.sql")
    ...
    task_member_rider_work_source = int_u.run_query("build_member_rider_work_source",
                                                    "dml/integration/incremental/dim_member_rider/mzp_member_rider_work_source.sql")
    task_member_rider_work_transformed = int_u.run_query("build_member_rider_work_transformed",
                                                         "dml/integration/incremental/dim_member_rider/mzp_member_rider_work_transformed.sql")
    task_member_rider_work_final_staging = int_u.run_query("build_member_rider_work_final_staging",
                                                           "dml/integration/incremental/dim_member_rider/mzp_member_rider_work_stage.sql")
    task_member_rider_merge = int_u.run_query('merge_member_rider',
                                              "dml/integration/incremental/dim_member_rider/dim_member_rider.sql")

    # Employee

    # Employee -Workarea Loads
    task_employee_work_source = int_u.run_query("build_employee_work_source","dml/integration/incremental/dim_employee/dim_employee_source.sql")
    task_employee_work_transformed = int_u.run_query("build_employee_work_transformed","dml/integration/incremental/dim_employee/mzp_employee_work_transformed.sql")
    task_employee_work_final_staging = int_u.run_query("build_employee_work_final_staging","dml/integration/incremental/dim_employee/employee_work_stage.sql")
    # Employee - Merge Load
    task_employee_merge = int_u.run_query('merge_employee',"dml/integration/incremental/dim_employee/dim_employee.sql")

    # Employee_role
    # Employee_role - Workarea loads
    task_employee_role_work_source = int_u.run_query("build_employee_role_work_source","dml/integration/incremental/dim_employee_role/mzp_employee_role_work_source.sql")
    task_employee_role_work_transformed = int_u.run_query("build_employee_role_work_transformed","dml/integration/incremental/dim_employee_role/mzp_employee_role_work_transformed.sql")
    task_employee_role_work_final_staging = int_u.run_query("build_employee_role_work_final_staging","dml/integration/incremental/dim_employee_role/mzp_employee_role_work_stage.sql")


# member - Merge Load
    task_employee_role_merge = int_u.run_query('merge_employee_role',"dml/integration/incremental/dim_employee_role/dim_employee_role.sql")
    ...
    task_membership_segment_source = int_u.run_query('membership_segment_source',
                                                     "dml/integration/incremental/dim_membership_marketing_segmentation/mzp_membership_segment_source.sql")
    task_membership_segment_transformed = int_u.run_query('membership_segment_transformed',
                                                          "dml/integration/incremental/dim_membership_marketing_segmentation/mzp_membership_segment_transformed.sql")
    task_membership_segment_stage = int_u.run_query('membership_segment_stage',
                                                    "dml/integration/incremental/dim_membership_marketing_segmentation/mzp_membership_segment_stage.sql")
    task_membership_segment_merge = int_u.run_query('membership_segment_merge',
                                                    "dml/integration/incremental/dim_membership_marketing_segmentation/dim_membership_marketing_segmentation.sql")

    # member_auto_renewal_card_
    task_member_auto_renewal_card_work_source = int_u.run_query('member_auto_renewal_card_work_source',
                                                                "dml/integration/incremental/dim_member_auto_renewal_card/mzp_auto_renewal_card_work_source.sql")
    task_member_auto_renewal_card_work_transformed = int_u.run_query('member_auto_renewal_card_work_transformed',
                                                                     "dml/integration/incremental/dim_member_auto_renewal_card/mzp_auto_renewal_card_work_transformed.sql")
    task_member_auto_renewal_card_final_stage = int_u.run_query('member_auto_renewal_card_final_stage',
                                                                "dml/integration/incremental/dim_member_auto_renewal_card/mzp_auto_renewal_card_work_stage.sql")
    task_merge_member_auto_renewal_card = int_u.run_query('merge_member_auto_renewal_card',
                                                          "dml/integration/incremental/dim_member_auto_renewal_card/dim_member_auto_renewal_card.sql")

    # billing_summary

    # billing_summary - Workarea loads
    task_billing_summary_work_source = int_u.run_query("build_billing_summary_work_source",
                                                       "dml/integration/incremental/membership_billing_summary/mzp_membership_billing_summary_work_source.sql")
    task_billing_summary_work_transformed = int_u.run_query("build_billing_summary_work_transformed",
                                                            "dml/integration/incremental/membership_billing_summary/mzp_membership_billing_summary_work_transformed.sql")
    task_billing_summary_work_final_staging = int_u.run_query("build_billing_summary_work_final_staging",
                                                              "dml/integration/incremental/membership_billing_summary/mzp_membership_billing_summary_work_stage.sql")

    # billing_summary - Merge Load
    task_billing_summary_merge = int_u.run_query("merge_billing_summary", "dml/integration/incremental/membership_billing_summary/membership_billing_summary.sql")
    ...
    task_membership_billing_detail_work_source = int_u.run_query("build_membership_billing_detail_work_source",
                                                                 "dml/integration/incremental/membership_billing_detail/mzp_membership_billing_detail_work_source.sql")
    task_membership_billing_detail_work_transformed = int_u.run_query(
        "build_membership_billing_detail_work_transformed",
        "dml/integration/incremental/membership_billing_detail/mzp_membership_billing_detail_work_transformed.sql")
    task_billing_detail_work_final_staging = int_u.run_query("build_billing_detail_work_final_staging",
                                                             "dml/integration/incremental/membership_billing_detail/mzp_membership_billing_detail_work_stage.sql")
    task_billing_detail_merge = int_u.run_query('merge_billing_detail',
                                                "dml/integration/incremental/membership_billing_detail/membership_billing_detail.sql")

    # membership_office

    # membership_office - Workarea loads
    task_membership_office_work_source = int_u.run_query("build_membership_office_work_source",
                                                         "dml/integration/incremental/dim_aca_office/mzp_aca_office_work_source.sql")
    task_membership_office_work_transformed = int_u.run_query("build_membership_office_work_transformed",
                                                              "dml/integration/incremental/dim_aca_office/mzp_aca_office_work_transformed.sql")
    task_membership_office_work_final_staging = int_u.run_query("build_membership_office_work_final_staging",
                                                                "dml/integration/incremental/dim_aca_office/mzp_aca_office_work_stage.sql")

    # membership_office - Merge Load
    task_membership_office_merge = int_u.run_query('merge_membership_office', "dml/integration/incremental/dim_aca_office/dim_aca_office.sql")

    # Payment Summary
    task_payment_applied_summary_source = int_u.run_query("payment_applied_summary_source",
                                                          "dml/integration/incremental/membership_payment_applied_summary/mzp_membership_payment_applied_summary_work_source.sql")
    task_payment_applied_summary_transformed = int_u.run_query("payment_applied_summary_transformed",
                                                               "dml/integration/incremental/membership_payment_applied_summary/mzp_membership_payment_applied_summary_work_transformed.sql")
    task_payment_summary_stage = int_u.run_query("payment_summary_stage", "dml/integration/incremental/membership_payment_applied_summary/mzp_membership_payment_applied_summary_work_stage.sql")
    task_payment_summary_merge = int_u.run_query('payment_summary_merge', "dml/integration/incremental/membership_payment_applied_summary/membership_payment_applied_summary.sql")
    task_payment_summary_reversal_update = int_u.run_query('payment_summary_reversal_update',
                                                           "dml/integration/incremental/membership_payment_applied_summary/payment_summary_reversal_update.sql")
    ...
    task_membership_gl_payments_applied_work_source = int_u.run_query("membership_gl_payments_applied_work_source",
                                                                      "dml/integration/incremental/dim_membership_gl_payments_applied/mzp_gl_payments_work_source.sql")
    task_membership_gl_payments_applied_work_transformed = int_u.run_query(
        "membership_gl_payments_applied_work_transformed","dml/integration/incremental/dim_membership_gl_payments_applied/mzp_gl_payments_work_transformed.sql" )
    task_membership_gl_payments_applied_work_final_staging = int_u.run_query(
        "membership_gl_payments_applied_work_final_staging", "dml/integration/incremental/dim_membership_gl_payments_applied/mzp_gl_payments_work_stage.sql")
    task_membership_gl_payments_applied_merge = int_u.run_query("membership_gl_payments_applied_merge",
                                                                "dml/integration/incremental/dim_membership_gl_payments_applied/dim_membership_gl_payments_applied.sql")
    ...
    task_membership_payments_received_work_source = int_u.run_query("Membership_payments_received_work_source",
                                                                    "dml/integration/incremental/membership_payments_received/mzp_membership_payments_received_work_source.sql")
    task_membership_payments_received_work_transformed = int_u.run_query(
        "Membership_payments_received_work_transformed",
        "dml/integration/incremental/membership_payments_received/mzp_payments_received_work_transformed.sql")
    task_membership_payments_received_work_final_staging = int_u.run_query(
        ...
        "dml/integration/incremental/membership_payments_received/mzp_payments_received_work_stage.sql")
    task_membership_payments_received_merge = int_u.run_query('merge_membership_payments_received',
                                                              "dml/integration/incremental/membership_payments_received/membership_payments_received.sql")

    # Membership_fee
    # Membership_fee - Workarea loads
    task_membership_fee_work_source = int_u.run_query("build_membership_fee_work_source","dml/integration/incremental/dim_membership_fee/mzp_membership_fee_work_source.sql")
    task_membership_fee_work_transformed = int_u.run_query("build_membership_fee_work_transformed",
                                                           "dml/integration/incremental/dim_membership_fee/mzp_membership_fee_work_transformed.sql")
    task_membership_fee_work_final_staging = int_u.run_query("build_membership_fee_work_final_staging",
                                                             "dml/integration/incremental/dim_membership_fee/mzp_membership_fee_work_stage.sql")
    # Membership_fee - Merge Load

    task_membership_fee_merge = int_u.run_query('merge_membership_fee', "dml/integration/incremental/dim_membership_fee/dim_membership_fee.sql")
    ...
    task_membership_solicitation_work_source = int_u.run_query("build_membership_solicitation_work_source",
                                                               "dml/integration/incremental/dim_membership_solicitation/mzp_solicitation_work_source.sql")
    task_membership_solicitation_work_transformed = int_u.run_query("build_membership_solicitation_work_transformed",
                                                                    "dml/integration/incremental/dim_membership_solicitation/mzp_solicitation_work_transformed.sql")
    task_membership_solicitation_work_final_staging = int_u.run_query(
        "build_membership_solicitation_work_final_staging",  "dml/integration/incremental/dim_membership_solicitation/mzp_solicitation_work_stage.sql")

    # membership_solicitation - Merge Load
    task_membership_solicitation_merge = int_u.run_query('merge_membership_solicitation',  "dml/integration/incremental/dim_membership_solicitation/dim_membership_solicitation.sql")

    # Payment Plan

    task_payment_plan_source = int_u.run_query('paymentplan_source',
                                               "dml/integration/incremental/dim_membership_payment_plan/mzp_membership_payment_plan_work_source.sql")
    task_payment_plan_transformed = int_u.run_query('paymentplan_transformed',
                                                    "dml/integration/incremental/dim_membership_payment_plan/mzp_membership_payment_plan_work_transformed.sql")
    task_payment_plan_stage = int_u.run_query('paymentplan_stage',
                                             "dml/integration/incremental/dim_membership_payment_plan/mzp_membership_payment_plan_work_stage.sql")
    task_payment_plan_merge = int_u.run_query('paymentplan_merge',
                                             "dml/integration/incremental/dim_membership_payment_plan/dim_membership_payment_plan.sql")
    ...
    task_member_pay_app_detail_work_source = int_u.run_query('build_member_pay_app_detail_work_source',
                                                             "dml/integration/incremental/membership_payment_applied_detail/mzp_membership_payment_applied_detail_work_source.sql")
    task_member_pay_app_detail_work_transformed = int_u.run_query('build_member_pay_app_detail_work_transformed',
                                                                  "dml/integration/incremental/membership_payment_applied_detail/mzp_membership_payment_applied_detail_work_transformed.sql")
    task_member_pay_app_detail_work_final_staging = int_u.run_query('build_member_pay_app_detail_work_final_staging',
                                                                    "dml/integration/incremental/membership_payment_applied_detail/mzp_membership_payment_applied_detail_work_stage.sql")
    task_membership_pay_app_detail_merge = int_u.run_query('merge_membership_pay_app_detail_merge',
                                                           "dml/integration/incremental/membership_payment_applied_detail/membership_payment_applied_detail.sql")

    # Sales_agent_Activity
    task_sales_agent_activity_work_source = int_u.run_query('sales_agent_activity_work_source',
                                                            "dml/integration/incremental/sales_agent_activity/mzp_sales_agent_activity_work_source.sql")
    task_sales_agent_activity_work_transformed = int_u.run_query('sales_agent_activity_work_transformed',
                                                                 "dml/integration/incremental/sales_agent_activity/mzp_sales_agent_activity_work_transformed.sql")
    task_sales_agent_activity_work_final_staging = int_u.run_query('sales_agent_activity_work_final_staging',
                                                                   "dml/integration/incremental/sales_agent_activity/mzp_sales_agent_activity_work_stage.sql")
    task_sales_agent_activity_merge = int_u.run_query('sales_agent_activity_merge', "dml/integration/incremental/sales_agent_activity/sales_agent_activity.sql")
    task_sales_agent_activity_membership_solicitation_insert = int_u.run_query(
        'sales_agent_activity_membership_solicitation_insert', "dml/integration/incremental/sales_agent_activity/sales_agent_activity_membership_solicitation_insert.sql")
    ...
    task_tgt_count = int_u.run_query('mzp_tgt_count', "dml/integration/mzp/record_count/tgt_count.sql")

    # Dim_contact_segmentation
    task_contact_segm_prep_work = int_u.run_query('dim_contact_segmentation_prep_work',
                                              "dml/integration/incremental/dim_contact_segmentation/dim_contact_segmentation_prep_work.sql")
    task_contact_segm_merge = int_u.run_query('dim_contact_segmentation_merge',
                                          "dml/integration/incremental/dim_contact_segmentation/dim_contact_segmentation.sql")

    task_contact_segm_sig_prep_work = int_u.run_query('dim_contact_segm_sig_prep_work',
                                              "dml/integration/incremental/dim_contact_segmentation_sigma/dim_contact_segmentation_sigma_prep_work.sql")
    task_contact_segm_sig_merge = int_u.run_query('dim_contact_segm_sig_merge',
                                          "dml/integration/incremental/dim_contact_segmentation_sigma/dim_contact_segmentation_sigma.sql")

######################################################################
    # DEPENDENCIES
#contact dependencies
#sensor: ingest_mzp
task_name_insert.set_upstream([sensor_ingest_mzp_task])
task_phone_insert.set_upstream([sensor_ingest_mzp_task])
task_address_insert.set_upstream([sensor_ingest_mzp_task])
task_email_insert.set_upstream([sensor_ingest_mzp_task])
task_contact_work_target.set_upstream([sensor_ingest_mzp_task])
task_product_source.set_upstream([sensor_ingest_mzp_task])
task_membership_solicitation_work_source.set_upstream([sensor_ingest_mzp_task])
task_employee_work_source.set_upstream([sensor_ingest_mzp_task])
task_membership_office_work_source.set_upstream([sensor_ingest_mzp_task])

#sensor: ingest_sigma_add_assoc_file
#task_name_insert.set_upstream([sensor_ingest_sigma_task])
#task_phone_insert.set_upstream([sensor_ingest_sigma_task])
#task_address_insert.set_upstream([sensor_ingest_sigma_task])
#task_email_insert.set_upstream([sensor_ingest_sigma_task])
#task_product_source.set_upstream([sensor_ingest_sigma_task])
#task_employee_work_source.set_upstream([sensor_ingest_sigma_task])
#task_membership_solicitation_work_source.set_upstream([sensor_ingest_sigma_task])
#task_membership_office_work_source.set_upstream([sensor_ingest_sigma_task])
...
# comments
[task_membership_merge,
  task_employee_merge] >> task_membership_comments_source >> task_membership_comment_fkey >> task_membership_comment_stage >> task_membership_comment
...
#Record count validation
task_sales_agent_activity_merge >> task_tgt_count
...
AllTaskSuccess.set_upstream([task_membership_comment, task_membership_gl_payments_applied_merge, task_sales_agent_activity_merge, task_payment_plan_merge, task_payment_summary_reversal_update, task_billing_detail_merge, task_membership_segment_merge,task_tgt_count, task_contact_segm_sig_merge])