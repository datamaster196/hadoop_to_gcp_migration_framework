 -- initial sourcing of data from ingest
CREATE OR REPLACE TABLE `adw-dev.adw_work.office_source`  AS
SELECT
Address as Address,
State as State,
Phone as Phone,
...
FROM
  `adw-lake-dev.mzp.aca_store_list` source
  WHERE
  CAST(CURRENT_DATE() AS datetime) > (
  SELECT
    MAX(effective_start_datetime)
  FROM
    `adw-dev.adw.dim_aca_office`)

	
	 --enhancement of data and foreign keying
CREATE OR REPLACE TABLE
  `adw-dev.adw_work.mzp_office_work_transformed` AS
  SELECT 
  Address,
State,
Phone,
...
ifnull(National_ATS_Number,''),'|'
  ...
  FROM `adw-dev.adw_work.office_source` AS office
  WHERE dupe_check =1
  
    ----------------------------------------------------------
  ----bringing all the data in staging area
CREATE OR REPLACE TABLE
  `adw-dev.adw_work.office_work_stage` AS
  SELECT
...
source.National_ATS_Number as national_ats_number,
  ...
  {{ dag_run.id }} integrate_update_batch_number
    FROM
  `adw-dev.adw_work.mzp_office_work_transformed` source
LEFT JOIN
  `adw-dev.adw.dim_aca_office` target
ON
  (source.code=SAFE_CAST(target.aca_office_list_source_key AS STRING)
...
MERGE INTO
  `adw-dev.adw.dim_aca_office` a
USING
  `adw-dev.adw_work.office_work_stage` b
