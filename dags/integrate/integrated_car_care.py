#######################################################################################################################
##  DAG_NAME :  integrated_car_care
##  PURPOSE :   Executes incremental load for car care (VAST) model
##  PREREQUISITE DAGs :
#######################################################################################################################
...
SENSOR_TIMEDELTA_VAST = Variable.get("integrated_care_care_vast_timedelta", default_var='') or None

DAG_TITLE = "integrated_car_care"
...
with DAG(DAG_TITLE, default_args=default_args, schedule_interval=SCHEDULE, catchup=False, max_active_runs=1 ,
    ...
    # TASKS
    # Sensor Task
    sensor_ingest_vast_task = ExternalTaskSensor(task_id='sensor_ingest_vast',
                                                 external_dag_id='ingest_vast',
                                                 external_task_id='AllTaskSuccess',
                                                 #execution_delta = timedelta(minutes=[SENSOR_TIMEDELTA_VAST]),
    ...
    task_contact_work_source = int_u.run_query("build_contact_work_source","dml/integration/incremental/contact_info/vast/contact_vast_work_source.sql")
    ...
    task_fact_sales_header_car_care_prep_work = int_u.run_query('fact_sales_header_car_care_prep_work',
                                         "dml/integration/incremental/fact_sales_header/car_care/fact_sales_header_car_care_prep_work.sql")
  
    task_fact_sales_header_car_care_merge = int_u.run_query('fact_sales_header_merge',
                                         "dml/integration/incremental/fact_sales_header/car_care/fact_sales_header_car_care.sql")
...
task_name_insert.set_upstream([sensor_ingest_vast_task])
task_phone_insert.set_upstream([sensor_ingest_vast_task])
task_address_insert.set_upstream([sensor_ingest_vast_task])
task_email_insert.set_upstream([sensor_ingest_vast_task])
task_contact_work_target.set_upstream([sensor_ingest_vast_task])
